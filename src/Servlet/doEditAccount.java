...
@WebServlet(urlPatterns = { "/doEditAccount" })
public class doEditAccount extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       try {
		cal.setTime(timeFormat.parse(time1));
	} catch (ParseException e1) {
		// TODO Auto-generated catch block
		e1.printStackTrace();
	}
       String errorString = null;
       String time =std.format(cal.getTime());
       String per = (String) request.getParameter("per");
       if ((!per.equals("admin"))&&(!per.equals("user"))) errorString="Permission is not correct!";
       ...
       try {
           DBUtility.ADupdateAccount(conn, account);
